...
const CONTROLLER_IMPORT_DECLARATION_AST_QUERY = CONTROLLER_NAMES.map(
  (controllerName) => `ImportDeclaration:has([name="${controllerName}"])`
).join(', ');

export const migrateControllerImports = (
  tree: Tree,
  ...
  tree.getDir('src').visit((filePath) => {
    if (filePath.endsWith('.tsx')) {
      ...
      if (fileContent) {
        ...
        const controllerImports = tsquery.query<ts.ImportDeclaration>(
          sourceFile,
          CONTROLLER_IMPORT_DECLARATION_AST_QUERY,
          { visitAllChildren: true }
        );
        if (controllerImports.length > 0) {
          context.logger.info(
            `Deprecated controller imports identified in: ${filePath}...`
          );
          /**
          ...
          for (const controllerImport of controllerImports) {
            const controllerNamedBindings =
              controllerImport.importClause?.namedBindings;
            ...
            if (controllerImportSpecifiers.length > 0) {
              const result = createTsxImportStatement(
                ...
                ts.factory.createNamedImports(controllerImportSpecifiers),
                fileContent
              );
              ...
              newContent = tsquery.replace(
                ...
                (node) => {
                  if (ts.isImportDeclaration(node)) {
                    return (
                      removeImportSpecifiers(node, ...CONTROLLER_NAMES) +
                      '\n' +
                      result
                    );
                  }
